...
| Файл с Google Drive | Имя в проекте |
|---------------------|---------------|
| CSV с резюме (в архиве может называться иначе) | `data/hh_resumes.csv` |
| `ExchangeRates.csv` | `data/exchange_rates.csv` |

Датасет в формате [HeadHunter](https://hh.ru) (открытый дамп для анализа). Только для портфолио и обучения, не для коммерческого парсинга.
